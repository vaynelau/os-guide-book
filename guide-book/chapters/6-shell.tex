...
实现shell的一个关键函数是spawn，其作用是接受可执行文件路径和传给其的参数，创建一个新的进程使得可执行文件的相关段被映射到新进程的内存中，并设置好栈，以及一些已初始化的数据，比如管道和重定向的文件描述符。其作用类似load_icode和fork，但实现方式不一样。相信经过之前的训练，同学们应该可以较为容易的填出spawn函数。

\begin{exercise}
  根据上面的表述以及函数内的注释，补充完成 user/spawn.c 中的\mintinline{c}|int spawn(char *prog, char **argv)|。
  完成后可以初步看见shell的现象。
\end{exercise}
