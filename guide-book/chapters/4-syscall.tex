...
首先我们看向sys\_mem\_alloc。这个函数的主要功能是分配内存，简单的说来，用户程序可以通过这个系统调用给该程序所允许的虚拟内存空间内存分配实际的物理内存，需要用到一些我们之前在pmap.c中所定义的函数
\begin{exercise}
实现lib/syscall\_all.c中的void sys\_mem\_alloc(int sysno,u\_int dstva)函数
\end{exercise}
